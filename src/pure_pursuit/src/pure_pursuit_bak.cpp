...
// PurePursuit::PurePursuit() : lookahead_distance_(1.0), v_max_(0.5), v_(v_max_), w_max_(1.0), position_tolerance_(0.1), idx_(0),
...
PurePursuit::PurePursuit() : lookahead_distance_(1.0), w_max_(1.0), position_tolerance_(0.1), idx_(0),
                             goal_reached_(false), nh_private_("~"), tf_listener_(tf_buffer_),
                             map_frame_id_("map"), robot_frame_id_("base_link"), lookahead_frame_id_("lookahead")
{
  ...
  obstacle_sub_ = nh_.subscribe("/front_obstacle_detect", 1, &PurePursuit::obstacle_status, this);
  emergency_stop_sub_ = nh_.subscribe("/emergency_stop", 1, &PurePursuit::emergency_stop_status, this);
...
void PurePursuit::obstacle_status(std_msgs::Bool status){
  obstacle_detected_ = status.data;
  ROS_INFO("Obstacle detected: %s", obstacle_detected_ ? "True" : "False");
}
...
void PurePursuit::cmd_generator(nav_msgs::Odometry odom){
  if(obstacle_detected_ || !canMove){ //emergency_stop_
    cmd_vel_.linear.x = 0.00;
    cmd_vel_.linear.y = 0.00;
    cmd_vel_.linear.z = 0.00;
    cmd_vel_.angular.x = 0.00;
    cmd_vel_.angular.y = 0.00;
  ...
  if (path_loaded_){
    ...
    try{
      ...
      const geometry_msgs::Vector3& translation = tf.transform.translation;
      ...
      const geometry_msgs::Quaternion& rotation = tf.transform.rotation;
      std::cout << "Transform Rotation (Quaternion): "
                << "x: " << rotation.x << ", "
                << "y: " << rotation.y << ", "
                << "z: " << rotation.z << ", "
                << "w: " << rotation.w << std::endl;
      for (idx_=idx_memory; idx_ < path_.poses.size(); idx_++){
          geometry_msgs::Pose pose = path_.poses[idx_].pose;
          std::cout << "Position: "
            << "x: " << pose.position.x << ", "
            << "y: " << pose.position.y << ", "
      ...
      if (!goal_reached_){
        ...
        cmd_vel_.angular.z = std::min(3*v_/lookahead_distance_*lookahead_distance_*lateral_offset, w_max_);
      ...
      lookahead_marker_.scale.x = 0.25;
      lookahead_marker_.scale.y = 0.25;
      lookahead_marker_.scale.z = 0.25;
